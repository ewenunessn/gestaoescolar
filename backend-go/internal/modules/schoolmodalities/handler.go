...
type ServiceContract interface {
	Create(context.Context, int64, CreateRequest) (SchoolEducationModality, error)
	ListBySchool(context.Context, int64, ListQuery) (ListResult, error)
	Update(context.Context, int64, int64, UpdateRequest) (SchoolEducationModality, error)
	Delete(context.Context, int64, int64) (SchoolEducationModality, error)
}
...
func (h Handler) List(w http.ResponseWriter, r *http.Request) {
	schoolID, ok := parseParamID(w, r, "schoolId")
	if !ok {
		return
	}
	...
	result, err := h.service.ListBySchool(r.Context(), schoolID, query)
	if err != nil {
		writeError(w, err)
		return
	}
...
func (h Handler) Create(w http.ResponseWriter, r *http.Request) {
	...
	item, err := h.service.Create(r.Context(), schoolID, request)
	if err != nil {
		writeError(w, err)
		return
	}
...
func (h Handler) Update(w http.ResponseWriter, r *http.Request) {
	...
	item, err := h.service.Update(r.Context(), schoolID, id, request)
	if err != nil {
		writeError(w, err)
		return
	}
...
func (h Handler) Delete(w http.ResponseWriter, r *http.Request) {
	...
	item, err := h.service.Delete(r.Context(), schoolID, id)
	if err != nil {
		writeError(w, err)
		return
	}
...
func writeError(w http.ResponseWriter, err error) {
	...
	case errors.Is(err, ErrNotFound):
		writeAPIError(w, http.StatusNotFound, "school_education_modality_not_found", "School education modality not found")
	case errors.Is(err, ErrConflict):
		writeAPIError(w, http.StatusConflict, "school_education_modality_conflict", "School already has this active education modality")
	default:
