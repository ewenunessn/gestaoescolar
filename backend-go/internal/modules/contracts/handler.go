...
type ServiceContract interface {
	...
	Delete(context.Context, int64) (Contract, error)
}
type Handler struct{ service ServiceContract }

func NewHandler(s ServiceContract) Handler { return Handler{s} }
...
func parseID(w http.ResponseWriter, r *http.Request) (int64, bool) {
	...
	if err != nil || id <= 0 {
		writeAPIErr(w, 400, "invalid_id", "id must be positive")
		return 0, false
	}
	return id, true
}
func parseList(w http.ResponseWriter, r *http.Request) (ListQuery, bool) {
	...
	if s := strings.TrimSpace(v.Get("status")); s != "" {
		st, ok := parseStatus(s)
		if !ok {
			writeAPIErr(w, 400, "invalid_status", "invalid status")
			return q, false
		}
		q.Status = &st
	}
	if s := strings.TrimSpace(v.Get("supplierId")); s != "" {
		n, err := strconv.ParseInt(s, 10, 64)
		if err != nil || n <= 0 {
			writeAPIErr(w, 400, "invalid_supplier", "supplierId must be positive")
			return q, false
		}
		q.SupplierID = &n
	}
...
func writeErr(w http.ResponseWriter, err error) {
	...
	case errors.Is(err, ErrNotFound):
		writeAPIErr(w, 404, "contract_not_found", "Contract not found")
	case errors.Is(err, ErrConflict):
		writeAPIErr(w, 409, "contract_conflict", "Contract already exists")
	default:
