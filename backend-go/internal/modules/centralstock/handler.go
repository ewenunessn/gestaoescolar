...
type ServiceContract interface {
	Create(context.Context, CreateRequest) (Movement, error)
...
func (h Handler) Balances(w http.ResponseWriter, r *http.Request) {
	items, err := h.service.Balances(r.Context())
	if err != nil {
		writeErr(w, err)
		return
	}
...
func writeErr(w http.ResponseWriter, err error) {
	...
	case errors.Is(err, ErrNotFound):
		writeAPIErr(w, 404, "central_stock_movement_not_found", "Central stock movement not found")
	case errors.Is(err, ErrInsufficientStock):
		writeAPIErr(w, 409, "insufficient_central_stock", "Central stock balance is insufficient")
	default:
