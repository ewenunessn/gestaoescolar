...
type ServiceContract interface {
	Create(context.Context, CreateRequest) (Menu, error)
	GetByID(context.Context, int64) (Menu, error)
	List(context.Context, ListQuery) ([]Menu, *int64, error)
	Update(context.Context, int64, UpdateRequest) (Menu, error)
	Delete(context.Context, int64) (Menu, error)
}
...
func parseList(w http.ResponseWriter, r *http.Request) (ListQuery, bool) {
	...
	if s := strings.TrimSpace(v.Get("year")); s != "" {
		n, err := strconv.Atoi(s)
		if err != nil || n <= 0 {
			writeAPIErr(w, 400, "invalid_year", "year must be positive")
			return q, false
		}
		q.Year = &n
	}
	if s := strings.TrimSpace(v.Get("month")); s != "" {
		n, err := strconv.Atoi(s)
		if err != nil || n < 1 || n > 12 {
			writeAPIErr(w, 400, "invalid_month", "month must be between 1 and 12")
			return q, false
		}
		q.Month = &n
	}
...
func writeErr(w http.ResponseWriter, err error) {
	...
	case errors.Is(err, ErrNotFound):
		writeAPIErr(w, 404, "menu_not_found", "Menu not found")
	case errors.Is(err, ErrConflict):
		writeAPIErr(w, 409, "menu_conflict", "Menu already exists")
	default:
