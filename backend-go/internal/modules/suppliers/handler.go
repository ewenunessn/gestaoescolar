...
type ServiceContract interface {
	...
	Delete(context.Context, int64) (Supplier, error)
}
...
func parseList(w http.ResponseWriter, r *http.Request) (ListQuery, bool) {
	...
	if s := strings.TrimSpace(v.Get("supplierType")); s != "" {
		t, ok := parseType(s)
		if !ok {
			writeAPIErr(w, 400, "invalid_supplier_type", "invalid supplierType")
			return q, false
		}
		q.SupplierType = &t
	}
...
func writeErr(w http.ResponseWriter, err error) {
	...
	case errors.Is(err, ErrNotFound):
		writeAPIErr(w, 404, "supplier_not_found", "Supplier not found")
	case errors.Is(err, ErrConflict):
		writeAPIErr(w, 409, "supplier_conflict", "Supplier already exists")
	default:
