...
type ServiceContract interface {
	Create(context.Context, int64, CreateRequest) (PreparationProduct, error)
	...
	Update(context.Context, int64, int64, UpdateRequest) (PreparationProduct, error)
	Delete(context.Context, int64, int64) (PreparationProduct, error)
}
...
func (h Handler) List(w http.ResponseWriter, r *http.Request) {
	preparationID, ok := parseParamID(w, r, "preparationId")
	if !ok {
		return
	}
	q, ok := parseList(w, r)
	...
	result, err := h.service.ListByPreparation(r.Context(), preparationID, q)
	if err != nil {
		writeErr(w, err)
		return
	}
	...
	writeJSON(w, 200, map[string]any{"data": result.Items, "meta": map[string]any{"limit": limit, "nextCursor": result.NextCursor, "hasMore": result.NextCursor != nil}})
}
...
func (h Handler) Get(w http.ResponseWriter, r *http.Request) {
	...
	item, err := h.service.GetByID(r.Context(), preparationID, id)
	if err != nil {
		writeErr(w, err)
		return
	}
...
func (h Handler) Create(w http.ResponseWriter, r *http.Request) {
	...
	item, err := h.service.Create(r.Context(), preparationID, req)
	if err != nil {
		writeErr(w, err)
		return
	}
...
func (h Handler) Update(w http.ResponseWriter, r *http.Request) {
	...
	item, err := h.service.Update(r.Context(), preparationID, id, req)
	if err != nil {
		writeErr(w, err)
		return
	}
...
func (h Handler) Delete(w http.ResponseWriter, r *http.Request) {
	...
	item, err := h.service.Delete(r.Context(), preparationID, id)
	if err != nil {
		writeErr(w, err)
		return
	}
...
func writeErr(w http.ResponseWriter, err error) {
	...
	case errors.Is(err, ErrNotFound):
		writeAPIErr(w, 404, "preparation_product_not_found", "Preparation product not found")
	case errors.Is(err, ErrConflict):
		writeAPIErr(w, 409, "preparation_product_conflict", "Preparation already has this active product per capita")
	default:
