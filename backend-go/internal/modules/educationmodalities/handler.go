...
type EducationModalityService interface {
	Create(context.Context, CreateRequest) (EducationModality, error)
	GetByID(context.Context, int64) (EducationModality, error)
	List(context.Context, ListQuery) (ListResult, error)
	Update(context.Context, int64, UpdateRequest) (EducationModality, error)
	Delete(context.Context, int64) (EducationModality, error)
}

type Handler struct {
	service EducationModalityService
}

func NewHandler(service EducationModalityService) Handler {
...
func (h Handler) Update(w http.ResponseWriter, r *http.Request) {
	...
	item, err := h.service.Update(r.Context(), id, request)
	if err != nil {
		writeError(w, err)
		return
	}
...
func parseListQuery(w http.ResponseWriter, r *http.Request) (ListQuery, bool) {
	values := r.URL.Query()
	query := ListQuery{Search: strings.TrimSpace(values.Get("search"))}
...
func writeError(w http.ResponseWriter, err error) {
	...
	case errors.Is(err, ErrNotFound):
		writeAPIError(w, http.StatusNotFound, "education_modality_not_found", "Education modality not found")
	case errors.Is(err, ErrConflict):
		writeAPIError(w, http.StatusConflict, "education_modality_conflict", "Education modality already exists")
	default:
