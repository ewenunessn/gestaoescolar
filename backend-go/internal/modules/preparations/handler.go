...
type ServiceContract interface {
	...
	Delete(context.Context, int64) (Preparation, error)
}
...
func parseList(w http.ResponseWriter, r *http.Request) (ListQuery, bool) {
	values := r.URL.Query()
	q := ListQuery{Search: strings.TrimSpace(values.Get("search"))}
...
func writeErr(w http.ResponseWriter, err error) {
	...
	case errors.Is(err, ErrNotFound):
		writeAPIErr(w, 404, "preparation_not_found", "Preparation not found")
	case errors.Is(err, ErrConflict):
		writeAPIErr(w, 409, "preparation_conflict", "Preparation already exists")
	default:
