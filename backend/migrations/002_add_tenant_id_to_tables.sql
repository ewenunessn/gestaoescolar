-- Migration: Add tenant_id to existing multi-tenant tables
-- Description: Adds tenant_id column to all tables that need multi-tenant isolation
-- Date: 2025-01-27

-- Add tenant_id to core business tables
-- These are the main tables that need tenant isolation
...
-- 9. ESTOQUE TABLES
-- Estoque Escolas (School Inventory)
...
-- Estoque Lotes (Inventory Batches)
...
-- Estoque Escolas Historico (School Inventory History)
...
-- Guia Produto Escola (Guide Product School)
...
-- Demandas Escolas (School Demands)
ALTER TABLE demandas_escolas ADD COLUMN IF NOT EXISTS tenant_id UUID REFERENCES tenants(id) ON DELETE CASCADE;
CREATE INDEX IF NOT EXISTS idx_demandas_escolas_tenant_id ON demandas_escolas(tenant_id);

-- 13. RELATIONSHIP TABLES
-- Escola Modalidades (School Modalities)
...
-- Escolas Modalidades (Schools Modalities - alternative table)
...
-- Contrato Produtos (Contract Products)
...
-- Contrato Produtos Modalidades (Contract Products Modalities)
...
-- Faturamento Itens (Billing Items)
...
-- Estoque Alertas (Inventory Alerts)
...
-- 16. REFEICAO PRODUTOS (Meal Products)
...
-- This assigns all existing records to the default system tenant
...
UPDATE demandas_escolas SET tenant_id = '00000000-0000-0000-0000-000000000000' WHERE tenant_id IS NULL;
...
-- These indexes include tenant_id as the first column for optimal filtering
