...
// Usar NEON_DATABASE_URL para produção
const pool = new Pool({
...
async function testar() {
  ...
  try {
    console.log('🔍 Testando detecção de produtos em múltiplos contratos (NEON/PRODUÇÃO)...\n');
    ...
    if (result.rows.length === 0) {
      ...
      // Verificar produtos por contrato
      console.log('\n📦 Produtos por contrato:');
      for (const contrato of contratosAtivos.rows) {
        const produtos = await client.query(`
          SELECT p.nome, cp.preco_unitario
        ...
        produtos.rows.forEach(p => {
          console.log(`    - ${p.nome} (R$ ${parseFloat(p.preco_unitario).toFixed(2)}/kg)`);
        });
      }
    ...
    // 4. Verificar produtos que aparecem em apenas 1 contrato (para comparação)
    const produtosUmContrato = await client.query(`
    ...
    console.log(`\n📊 Resumo:`);
    console.log(`  - Produtos em múltiplos contratos: ${result.rows.length}`);
    console.log(`  - Produtos em apenas 1 contrato: ${produtosUmContrato.rows[0].total}`);
