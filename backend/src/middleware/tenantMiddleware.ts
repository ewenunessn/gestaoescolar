...
export function tenantMiddleware(options: TenantMiddlewareOptions = {}) {
  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // 2. Tentar por header X-Tenant-ID (PRIORIDADE MÁXIMA)
      ...
      if (tenantHeader) {
        ...
        if (result.tenant) {
          tenant = result.tenant;
          method = 'header';
          console.log('✅ Tenant resolvido por header (prioridade):', tenant.name);
        } else {
          console.log('❌ Não foi possível resolver tenant por header');
        }
      }

      // 3. Tentar por token JWT (apenas se não tiver header)
      if (!tenant) {
      ...
      if (tenant) {
        ...
        // Log da resolução (apenas em desenvolvimento)
        if (process.env.NODE_ENV === 'development') {
          console.log(`🏢 Tenant resolvido: ${tenant.name} (${tenant.slug}) via ${method}`);
        }
      }
