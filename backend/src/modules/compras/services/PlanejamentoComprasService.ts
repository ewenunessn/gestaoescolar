...
export const gerarPedidosPorPeriodo = async (reqBody: any, usuarioId: number, reqQuery?: any, reqParam?: any) => {
  const { competencia, periodos, escola_ids, observacoes } = reqBody as {
    competencia: string;
    periodos: Periodo[];
    escola_ids?: number[];
    observacoes?: string;
  ...
  try {
    ...
    const { contratos_selecionados } = reqBody;
...
export const calcularDemandaPorCompetencia = async (reqBody: any, usuarioId: number, reqQuery?: any, reqParam?: any) => {
  const { competencia, data_inicio, data_fim, escola_ids } = reqBody as {
  ...
  try {
    if (!competencia) {
      return { status: 400, data: { error: 'Competência é obrigatória' } };
    }

    if (!data_inicio || !data_fim) {
      return { status: 400, data: { error: 'Período é obrigatório' } };
    }
    ...
    return { status: 500, data: { error: 'Erro ao calcular demanda de compras' } };
  }
};
...
export const gerarPedidoDaGuia = async (reqBody: any, usuarioId: number, reqQuery?: any, reqParam?: any) => {
  ...
  if (!guia_id) {
    return { status: 400, data: { error: 'guia_id é obrigatório' } };
  }
  ...
  const client = await db.pool.connect();  try {
    ...
    if (guiaResult.rows.length === 0) {
      await client.query('ROLLBACK');
      return { status: 404, data: { error: 'Guia não encontrada' } };
    }
    ...
    if (itensResult.rows.length === 0) {
      await client.query('ROLLBACK');
      return { status: 400, data: { error: 'A guia não possui itens com quantidade > 0' } };
    }
    ...
    const { ignorar_sem_contrato } = reqBody;
    ...
    return { status: 500, data: { error: error instanceof Error ? error.message : 'Erro interno' } };
  } finally {
    client.release();
  }
};
...
export const iniciarGeracaoGuias = async (reqBody: any, usuarioId: number, reqQuery?: any, reqParam?: any) => {
  ...
  try {
    ...
    return { status: 500, data: { error: 'Erro ao iniciar geração de guias' } };
  }
};
...
export const buscarStatusJob = async (reqBody: any, usuarioId: number, reqQuery?: any, reqParam?: any) => {
  const { id } = reqParam;
  ...
  try {
    ...
    if (!job) {
      return { status: 404, data: { error: 'Job não encontrado' } };
    }

    return { status: 200, data: job };
  } catch (error) {
    console.error('Erro ao buscar job:', error);
    return { status: 500, data: { error: 'Erro ao buscar status do job' } };
  }
};

// ─── Listar Jobs do Usuário ───────────────────────────────────────────────────
export const listarJobsUsuario = async (reqBody: any, usuarioId: number, reqQuery?: any, reqParam?: any) => {
  try {
    const usuario_id = usuarioId;
    
    if (!usuario_id) {
      return { status: 401, data: { error: 'Usuário não autenticado' } };
    }
    ...
    return { status: 500, data: { error: 'Erro ao listar jobs' } };
  }
};


// ─── Iniciar Job de Geração de Pedido (Assíncrono) ───────────────────────────
export const iniciarGeracaoPedido = async (reqBody: any, usuarioId: number, reqQuery?: any, reqParam?: any) => {
  const { guia_id, observacoes, contratos_selecionados, ignorar_sem_contrato } = reqBody;
  ...
  try {
    ...
    return { status: 500, data: { error: 'Erro ao iniciar geração de pedido' } };
  }
};
