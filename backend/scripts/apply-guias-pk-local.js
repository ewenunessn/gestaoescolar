...
async function aplicarPKLocal() {
  try {
    console.log('🔧 Aplicando PRIMARY KEY na tabela guias (LOCAL)...\n');
    ...
    if (checkPK.rows.length > 0) {
      console.log('✅ PRIMARY KEY já existe na tabela guias (LOCAL)');
    } else {
      // Adicionar PRIMARY KEY
      await pool.query('ALTER TABLE guias ADD PRIMARY KEY (id)');
      console.log('✅ PRIMARY KEY adicionada com sucesso na tabela guias (LOCAL)');
    }
