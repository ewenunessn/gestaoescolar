...
async function listarTodasEntregas() {
  try {
    console.log('🔍 Listando TODAS as entregas em historico_entregas...\n');
    ...
    console.log(`📦 Total de entregas: ${result.rows.length}\n`);

    if (result.rows.length === 0) {
      console.log('❌ Nenhuma entrega encontrada na tabela historico_entregas');
      return;
    }

    console.log('📋 Todas as entregas:');
    console.log('═'.repeat(120));
    
    result.rows.forEach((row, index) => {
      console.log(`\n${index + 1}. ID: ${row.id} | Item: ${row.guia_produto_escola_id}`);
      ...
      console.log(`   Observação: ${row.observacao || '-'}`);
