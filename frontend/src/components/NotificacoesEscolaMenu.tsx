...
import {
  ...
  ListItemText, Tooltip, Divider, CircularProgress,
} from '@mui/material';
import {
...
import { useNotificacoesEscola } from '../contexts/NotificacoesEscolaContext';

type NotificacoesEscolaMenuProps = {
...
export default function NotificacoesEscolaMenu({ renderTrigger, placement = 'default' }: NotificacoesEscolaMenuProps) {
  const { notificacoes, naoLidas, loading, marcarTodasLidas, deletar, abrirNotificacao } = useNotificacoesEscola();
      ...
      >
        <Box sx={{ px: 2, py: 1.5, display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid', borderColor: 'divider' }}>
          <Typography variant="subtitle1" fontWeight={600}>
            Notificações{naoLidas > 0 && <Typography component="span" variant="caption" color="error.main" sx={{ ml: 0.5 }}>({naoLidas} não lidas)</Typography>}
          </Typography>
          {naoLidas > 0 && (
