...
import {
  ...
  calcularCustoCardapio, CustoCardapio, carregarTiposRefeicao
...
const CardapioCalendarioPage: React.FC = () => {
  ...
  const handleDiaClick = (data: string) => {
    ...
    // Bloquear domingo
    const dataClicada = dateUtils.createDate(anoClicado, mesClicado, diaClicado);
    if (dataClicada.getDay() === 0) {
      toast.warning('Não é permitido adicionar refeições aos domingos');
      return;
    }
          ...
          {diasSelecionadosCalendario.length > 0 && (
            <Box sx={{ mt: 2, display: 'flex', flexWrap: 'wrap', gap: 1, justifyContent: 'center' }}>
              {diasSelecionadosCalendario
                ...
                .map((data, index) => (
                  <Chip
                    key={`${data.getTime()}-${index}`}
                    label={`Dia ${data.getDate()}`}
                    onDelete={() => {
                      setDiasSelecionadosCalendario(
                        diasSelecionadosCalendario.filter(d => d.getTime() !== data.getTime())
                      );
                    }}
                    color="primary"
                    size="small"
